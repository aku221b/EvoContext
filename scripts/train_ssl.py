...
import pandas as pd
import math
from .utils1 import seed_torch, get_logger, count_parameters, load_model_checkpoint, CheckpointSaver, AverageMeter, compute_regression_loss
from .data_utils import *
from .dataloader_ssl import load_dataset_ssl
from .constants import *
from .args_ssl import get_args
from collections import OrderedDict
from json import dumps
from model.DCRNN import STAGModel_nextTimePred
...
def main(args):
# /Users/aksagaga/Desktop/Personal_Learning/STAGSMOTE/data/file_markers_chb_mit_ssl/trainSet_seq2seq_12s.txt
# /Users/aksagaga/Desktop/Personal_Learning/STAGSMOTE/eeg-gnn-ssl/data/file_markers_chb_mit_ssl/trainSet_seq2seq_12s.txt
    # Get device
    ...
    # args.save_dir = utils.get_save_dir(args.save_dir,args.dataset, "ssl", args.max_seq_len, "stagssmote", "", args.rand_seed)
    ...
    model = STAGModel_nextTimePred(device=device, args=args)
    ...
    if args.do_train:
        ...
        model = load_model_checkpoint(best_path, model)
        if isinstance(model, tuple):
            model = model[0]
        model = model.to(device)
...
def train(
    ...
    while (epoch != args.num_epochs) and (not early_stop):
        ...
        with torch.enable_grad(), \
            ...
            for x, y, _, supports, adj_mat_seq, _ in train_loader:
                ...
                adj_mat_seq = adj_mat_seq.to(device)
                ...
                seq_preds = model(x, y, adj_mat_seq, supports, batches_seen=step)
...
def evaluate(model, dataloader, args, save_dir, device, is_test=False,
    ...
    with torch.no_grad(), tqdm(total=len(dataloader.dataset)) as progress_bar:
        for x, y, _, supports, adj_mat_seq, file_name in dataloader:
            batch_size = x.shape[0]
            ...
            # Forward
            # (batch_size, output_seq_len, num_nodes, output_dim)
            seq_preds = model(x, y,adj_mat_seq, supports)
