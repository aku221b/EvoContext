...
from .utils2 import seed_torch,get_logger,get_save_dir,load_model_checkpoint,build_finetune_model,count_parameters,eval_dict,thresh_max_f1,AverageMeter,CheckpointSaver
from .data_utils import *
from .dataloader_chb import load_dataset_chb
from .constants import *
from .args import get_args
from collections import OrderedDict
from json import dumps
from model.DCRNN import STAGModel_classification, STAGModel_nextTimePred
...
def main(args):
    ...
    args.save_dir = get_save_dir(
       args.save_dir, args.dataset, "detection", args.max_seq_len, "EvoContext", "", args.rand_seed)
    # Save args
    ...
    if args.task == 'detection':
        dataloaders, _, scaler = load_dataset_chb(
            task=args.task,
            input_dir=args.input_dir,
            final_label_dir=args.final_label_dir,
            cached_graphs_dir=args.cached_graphs_dir,
    ...
    model = STAGModel_classification(
        args=args, num_classes=args.num_classes, device=device)
    # model = EvoBrain_classification(
    #     args=args, num_classes=args.num_classes, device=device)
    ...
    if args.do_train:
        if not args.fine_tune:
            ...
            if args.load_model_path is not None:
                ...
                pretrained_model = STAGModel_nextTimePred(
                    args=args_pretrained, device=device)  # placeholder
                pretrained_model = load_model_checkpoint(
...
def train(model, dataloaders, args, device, save_dir, log, tbx):
    ...
    while (epoch != args.num_epochs) and (not early_stop):
        ...
        with torch.enable_grad(), \
            ...
            for x, y, seq_lengths, supports, _, in train_loader:
...
def evaluate(
    ...
    with torch.no_grad(), tqdm(total=len(dataloader.dataset)) as progress_bar:
        for x, y, seq_lengths, supports, file_name in dataloader:
            batch_size = x.shape[0]
    ...
    # if (args.task == "detection") and (eval_set == 'dev'):
    best_thresh = thresh_max_f1(y_true=y_true_all, y_prob=y_prob_all)
    # update dev set y_pred based on best_thresh
    y_pred_all = (y_prob_all > best_thresh).astype(int)  # (batch_size, )
    # else:
    #     best_thresh = best_thresh

    scores_dict, _, _ = eval_dict(y_pred=y_pred_all,
                                        y=y_true_all,
                                        y_prob=y_prob_all,
