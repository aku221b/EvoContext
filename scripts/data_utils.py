...
def setup_logger(name, log_file='app.log', level=logging.INFO):
    ...
    # Avoid duplicate handlers if this logger is called multiple times
    if logger.handlers:
        return logger
...
def getOrderedChannels(file_name, verbose, labels_object, channel_names):
    logger = setup_logger("some_logger")
    labels = list(labels_object)
    # labels = [label for label in labels if label != "-" and label != "." and label != "ECG" and label != "VNS"]
    # if len(labels) != 23:
    #     logger.info(f"{file_name}_{len(labels)}")
    #     logger.info(labels_object)
    ...
    for ch in channel_names:
        try:
            ...
            if verbose:
                print(f"{file_name} failed to get channel '{ch}' — {str(e)}")
                print(f"Available labels: {labels}")
            raise Exception(f"Channel '{ch}' not found in labels for file {file_name}")
