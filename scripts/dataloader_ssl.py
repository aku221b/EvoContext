...
from .utils1 import StandardScaler
from .constants import INCLUDED_CHANNELS, FREQUENCY
from .data_utils import *
from .utils1 import calculate_scaled_laplacian, calculate_random_walk_matrix
...
def computeSliceMatrix(
    ...
    with h5py.File(h5_fn, 'r') as f:
        signal_array = f["filtered_signal"][()]  # type: ignore
        sampled_freq = f["filtered_freq"][()]  # type: ignore
    assert sampled_freq == FREQUENCY
    ...
    curr_slc = signal_array[:, start_window:end_window]  # type: ignore

    start_time_step = 0
    time_steps = []
    while start_time_step <= curr_slc.shape[1] - physical_time_step_size:  # type: ignore
        end_time_step = start_time_step + physical_time_step_size
        # (num_channels, physical_time_step_size)
        curr_time_step = curr_slc[:, start_time_step:end_time_step]  # type: ignore
        if is_fft:
...
class SeizureDataset(Dataset):
    def __init__(
        ...
        self.sensor_ids=INCLUDED_CHANNELS
    ...
    def _get_combined_graph(self, swap_nodes=None):
        ...
        """
        with open(self.adj_mat_dir, 'rb') as pf:  # type: ignore
    ...
    def _compute_supports(self, adj_mat):
        ...
        if self.filter_type == "laplacian":  # ChebNet graph conv
            supports_mat.append(
                calculate_scaled_laplacian(adj_mat, lambda_max=None))  # type: ignore
    ...
    def __getitem__(self, idx):
        ...
        """
        h5_fn, clip_idx_x, clip_idx_y = self.file_tuples[idx]

        cache_file_name = h5_fn.replace('.h5', '_cache.h5')
        cache_dir_exp = os.path.join(self.cache_dir, str(self.max_seq_len), self.filter_type)
        os.makedirs(cache_dir_exp, exist_ok=True)
        cache_file_path = os.path.join(cache_dir_exp, cache_file_name)
        # preprocess
        # if self.preproc_dir is None:
        resample_sig_dir = os.path.join(self.input_dir, h5_fn)
        eeg_clip_x = computeSliceMatrix(
            h5_fn=resample_sig_dir, clip_idx=int(clip_idx_x),
            time_step_size=self.time_step_size, clip_len=self.input_len,
            is_fft=self.use_fft)
        eeg_clip_y = computeSliceMatrix(
            h5_fn=resample_sig_dir, clip_idx=int(clip_idx_y),
            time_step_size=self.time_step_size, clip_len=self.output_len,
            is_fft=self.use_fft)
        # else:
        ...
        if self.standardize:
            x_feature = self.scaler.transform(x_feature)  # type: ignore
            y_feature = self.scaler.transform(y_feature)  # type: ignore
        ...
        indiv_combined_adj_mat = self._get_indiv_graphs(eeg_clip_x, swap_nodes)
        indiv_supports = self._compute_supports(indiv_combined_adj_mat)
        curr_support = np.concatenate(indiv_supports, axis=0)
        if np.any(np.isnan(curr_support)):
            raise ValueError("Nan found in indiv_supports!")
        ...
        if os.path.exists(cache_file_path):
            with h5py.File(cache_file_path, 'r') as cache_file:
                ...
                adj_mat_seq = torch.from_numpy(cache_file['adj_mats'][:])  # type: ignore
        else:
            ...
            for time_step in range(eeg_clip_x.shape[0]):
                adj_mat = self._get_indiv_graphs(eeg_clip_x[time_step][np.newaxis, :], swap_nodes)
            ...
            # print(f"adj_mat_seq -: {adj_mat_seq.shape}")
            # print(f"eeg_clip_x -: {eeg_clip_x.shape}")
        ...
        return (x, y, seq_len, curr_support, adj_mat_seq, writeout_fn)
