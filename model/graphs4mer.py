...
def get_knn_graph(x, k, dist_measure="cosine", undirected=True):
    ...
    # to sparse graph
    adj_mat_copy = adj_mat.clone().detach()
    edge_index, edge_weight = torch_geometric.utils.dense_to_sparse(adj_mat_copy)
...
class GraphS4mer(nn.Module):
    ...
    def forward(
        ...
        """
        # print(x.shape)
        batch, seq_len, num_nodes, dim = x.shape
        x = x.permute(0, 2, 1, 3).reshape(batch * num_nodes, seq_len * dim, 1)  # (batch * num_nodes, seq_len, 1)
        # print(x.shape)
        seq_len = seq_len * dim
        self.resolution = int (seq_len / 6)
        # self.resolution = int (seq_len / self.max_seq_len)
        # self.resolution = seq_len
...
class GraphS4mer_Regression(nn.Module):
    ...
    def forward(
        ...
        """
        print(x.shape)
        time, batch, num_node, dim = x
        x = x.permute(1, 2, 0).reshape(batch * num_nodes, time, 1)  # (batch * num_nodes, seq_len, 1)
        print(x.shape)
        batch = x.shape[0] // self.num_nodes
        num_nodes = self.num_nodes
        _, seq_len, _ = x.shape
        ...
        if self.residual_weight > 0:
            adj_mat = (
                self.residual_weight * adj_mat.clone() + (1 - self.residual_weight) * attn_weight
            )
        else:
            adj_mat = attn_weight
