...
class EvoBrain(nn.Module):
    ...
    """
    def __init__(self, feat_input_size_edge, feat_input_size_node, feat_target_size, embed_inside_size, convolve, reduce_edge, reduce_node, skip, activate, concat, neo_gnn, num_eigenvectors=0):
    ...
    def forward(self, inputs, supports):
        ...
        outputs = []
        for i in range(b):
            current_node_embeds = all_node_embeds[:, i].to(next(self.parameters()).device)
            current_edge_embeds = all_edge_embeds[:, i].to(next(self.parameters()).device)
            
            edge_weights = self.edge_activate(self.edge_transform(current_edge_embeds[-1]))

            # Apply Laplacian PE after SNN processing
            if self.num_eigenvectors > 0:
                data = Data(
                    x=current_node_embeds[-1].detach(),  # Use last timestep features
                    edge_index=edge_tuples.detach(),
                    edge_attr=current_edge_embeds[-1].detach(),  # Include edge embeddings
                    edge_weight=edge_weights.detach()  # Include edge weights for Laplacian computation
                )
                # if data.x is not None:
                    # print(f"N :- {data.x.size(0)}, k: ")
                
                data = self.laplacian_pe(data)
                
                # Combine SNN output with PE
                pe_features = data.laplacian_eigenvector_pe.to(current_node_embeds[-1].device)
            ...
            node_embeds = self.gnnx2.forward(
                edge_tuples, 
                edge_weights,
                self.activate(node_features_with_pe) if self.num_eigenvectors > 0 else self.activate(current_node_embeds[-1])
            )
...
class EvoBrain_classification(nn.Module):
    ...
    """
    def __init__(self, args, num_classes, device=None, gnn="ssg", num_eigenvectors=0):
